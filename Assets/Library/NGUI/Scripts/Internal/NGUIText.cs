...
static public class NGUIText
{
	...
	/// </summary>
	static public void NewPrint (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		if (string.IsNullOrEmpty(text)) return;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			prevX = x;
			//UnityEngine.Debug.Log("ch="+ch+" prevX="+prevX);
			// New line character -- skip to the next line
			if (ch == '\n')
			{
				if (x > maxX) maxX = x;
			...
			if (symbol != null)
			{
				...
				float w = glyph.advance * fontScale;
				if (finalSpacingX < 0f) w += finalSpacingX;
				//	if (Mathf.RoundToInt(x + w) > regionWidth)
			...
			else // No symbol present
			{
				...
				//UnityEngine.Debug.Log("capY="+ capY+" v0x="+v0x+" v1x="+v1x+" v0y="+v0y+" v1y="+v1y);
				
				//UnityEngine.Debug.Log("ch"+ch);
				if (IsSpace(ch))
				{
					if (underline)
					{
				...
				if (uvs != null)
				{
					...
					for (int j = 0, jmax = (bold ? 4 : 1); j < jmax; ++j)
					{
						...
						//	UnityEngine.Debug.Log(" glyph.u0="+glyph.u0.x+", "+glyph.u0.y+" glyph.u1="+glyph.u1.x+", "+glyph.u1.y+" glyph.u2="+glyph.u2.x+", "+glyph.u2.y+" glyph.u3="+glyph.u3.x+", "+glyph.u3.y);
					}
				}
				...
				//==============================
				float centerX = v0x+((v1x-v0x)/2.0f);
				float centerY = v0y+((v1y-v0y)/2.0f);	
				// centerY要置中
				if(centerY + (w/2.0f) > capY)
					centerY = capY - (w/2.0f);

				//UnityEngine.Debug.Log("capY="+capY+" centerX="+centerX+" centerY="+centerY + " finalLineHeight="+finalLineHeight);
				float dx = (v1y-v0y)/2.0f;
				float dy = (v1x-v0x)/2.0f; 

				v0x = Mathf.Round(centerX - dx); 
				v1x = Mathf.Round(centerX + dx);
				v0y = Mathf.Round(centerY-dy);
				v1y = Mathf.Round(centerY+dy);
				...
				if (!bold)
				{
					if (!italic)
					{
						verts.Add(new Vector3(v1x, v0y)); // move to first 
						verts.Add(new Vector3(v0x, v0y));
						verts.Add(new Vector3(v0x, v1y));
						verts.Add(new Vector3(v1x, v1y));
						
						//verts.Add(new Vector3(v1x, v0y));
						//UnityEngine.Debug.Log("  2 v0x="+v0x+" v1x="+v1x+" v0y="+v0y+" v1y="+v1y);
		...
		//UnityEngine.Debug.Log("indexOffset="+indexOffset);
		mColors.Clear();
	}
